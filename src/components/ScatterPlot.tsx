...
export default function ScatterPlot({
  ...
  const covariance =
    data.reduce((sum, d) => sum + (d.x - avgX) * (d.y - avgY), 0) / data.length;
  const stdX = Math.sqrt(
    data.reduce((sum, d) => sum + Math.pow(d.x - avgX, 2), 0) / data.length
  );
  const stdY = Math.sqrt(
    data.reduce((sum, d) => sum + Math.pow(d.y - avgY, 2), 0) / data.length
  );
          ...
          <div className="relative aspect-square max-h-80">
            ...
            {hoveredPoint !== null && (
              <div
                ...
                style={{
                  left: `${normalizeX(data[hoveredPoint].x)}%`,
                  top: `${normalizeY(data[hoveredPoint].y)}%`,
              ...
              >
                ...
                <p className="font-mono">
                  {xLabel}: {data[hoveredPoint].x}
                </p>
                <p className="font-mono">
                  {yLabel}: {data[hoveredPoint].y}
                </p>
              </div>
            )}
          </div>
